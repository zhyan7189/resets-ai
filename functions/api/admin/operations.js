  ...
  try {
    if (id) {
      ...
      const article = await env.DB.prepare(`SELECT a.id,'DA' || ai.number AS archive_code,a.title,a.card_title,a.status,a.author,a.created_at,a.updated_at,a.source_url,
        COALESCE(c.clicks,0) AS clicks,CAST(julianday('now')-julianday(a.created_at) AS INTEGER) AS retained_days
        FROM articles a LEFT JOIN archive_ids ai ON ai.article_id=a.id LEFT JOIN article_clicks c ON c.article_id=a.id WHERE a.id=?`).bind(id).first();
    ...
    const count = await env.DB.prepare("SELECT COUNT(*) AS total FROM articles a LEFT JOIN archive_ids ai ON ai.article_id=a.id WHERE a.title LIKE ? OR a.card_title LIKE ? OR ('DA' || ai.number) LIKE ?").bind(search, search, search).first();
    const rows = await env.DB.prepare(`SELECT a.id,'DA' || ai.number AS archive_code,a.title,a.card_title,a.status,a.author,a.created_at,a.updated_at,
      ...
      WHERE a.title LIKE ? OR a.card_title LIKE ? OR ('DA' || ai.number) LIKE ? ORDER BY a.created_at DESC LIMIT 20 OFFSET ?`).bind(search, search, search, (page - 1) * 20).all();
    return json({ rows:rows.results || [], total:count?.total || 0, page });
