...
export async function onRequestPost({ request, env }) {
  try {
    ...
    try {
      await env.DB.prepare(`INSERT INTO article_clicks_hourly (article_id,hour,clicks)
        VALUES (?,strftime('%Y-%m-%dT%H:00:00Z','now'),1)
        ON CONFLICT(article_id,hour) DO UPDATE SET clicks=clicks+1`).bind(articleId).run();
    } catch { /* 旧库尚未升级时仍保留原有累计点击统计。 */ }
