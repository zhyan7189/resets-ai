...
首页右上角的“控制台”进入 `/login`，验证管理密钥后进入 `/admin`。粘贴 HTTPS 链接后会建立导入记录和待审核内容：通用文章页尝试读取正文段落及图片顺序，JSON-LD 可提供额外正文，YouTube 链接可作为站内视频；配置 R2 后自动将可读取的封面和文章图片存入本站，无需手动上传。若设置 `AI_API_URL`、`AI_API_KEY`、`AI_MODEL`，还会生成中文卡片草稿，但不会改写原文。读取失败或素材不全时进入“需协助”，留下缺项说明，不会自动发布。

控制台包含驾驶舱、内容管理、审核管理、媒体库、运营中心和广告位。驾驶舱显示已发布、待审核、需协助、累计阅读、累计访问及近 30 天趋势，并显示 D1、R2 等连接状态。内容管理支持搜索、按状态筛选、每页 10 或 20 条、新增、编辑、下架和移入已废弃；新增内容可通过链接导入或手工录入。保存并提交审核后，管理员可在审核管理预览卡片和站内详情，填写拒绝原因或审核通过并发布。媒体库按内容展示已引用的 R2 文件及外部媒体，广告位用首页平面图管理四个位置。
...
后台需要 D1 绑定 `DB` 和加密环境变量 `ADMIN_TOKEN`。旧数据库需先运行一次 [v2 数据迁移](db/migrations/001-content-console-v2.sql)；已有 v2 数据库升级驾驶舱时需在备份后执行 [002 迁移](db/migrations/002-dashboard-analytics.sql)。新数据库使用 [完整表结构](db/schema.sql)。未配置时首页保留既有静态卡片，但后台不能保存内容。完整配置见 [DEPLOY.md](DEPLOY.md)。管理密钥不要写入仓库或公开给访客。模型接口为可选项，其调用费用不包含在 Cloudflare 免费托管额度内。
