...
export async function reader(request, env) {
  ...
  return env.DB.prepare("SELECT u.id, u.username FROM reader_sessions s JOIN reader_users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at > datetime('now')").bind(digest).first();
}
...
export async function startSession(userId, env, request) {
  ...
  return `${COOKIE}=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${SESSION_SECONDS}${new URL(request.url).protocol === "https:" ? "; Secure" : ""}`;
}
